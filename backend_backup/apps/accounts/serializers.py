...
class CounsellorProfileSerializer(serializers.ModelSerializer, UserProfilePartialSerializer):
    # Accept lists of primary keys for specializations & availability
    specializations = serializers.PrimaryKeyRelatedField(
        queryset=Specialization.objects.all(), many=True, required=False
    )
    availability = serializers.PrimaryKeyRelatedField(
        queryset=AvailabilitySlot.objects.all(), many=True, required=False
    )
    ...
    def update(self, instance, validated_data):
        ...
        if availability is not None:
            instance.availability.set(availability)
